...
export class Validator {
    ...
    checkValidity() {
        const parentheses = this.tokens.filter((token) => token.type === 'openParenthesis' || token.type === 'closeParenthesis');
        const brackets = this.tokens.filter((token) => token.type === 'openBracket' || token.type === 'closeBracket');
        const curlyBraces = this.tokens.filter((token) => token.type === 'openCurlyBrace' || token.type === 'closeCurlyBrace');

        const conditionalTokens = this.tokens.filter((token) => token.type === 'conditionalIf' || token.type === 'conditionalThen');

        const parenthesesCountMatches = this.checkParenthesesCountMatches(parentheses);
        let parenthesesOrderingIsOk = true;
        if (parenthesesCountMatches) {
            parenthesesOrderingIsOk = this.checkParenthesesOrdering(parentheses);
        }

        const bracketCountMatches = this.checkBracketCountMatches(brackets);
        let bracketOrderingIsOk = true;
        if (bracketCountMatches) {
            bracketOrderingIsOk = this.checkBracketOrdering(brackets);
        }

        const curlyBraceCountMatches = this.checkCurlyBraceCountMatches(curlyBraces);
        let curlyBraceOrderingIsOk = true;
        if (curlyBraceCountMatches) {
            curlyBraceOrderingIsOk = this.checkCurlyBraceOrdering(curlyBraces);
        }

        const conditionalCountMatches = this.checkConditionalCountMatches(conditionalTokens);
        const onlyLiteralsBetweenCurlyBraces = this.checkOnlyLiteralsBetweenCurlyBraces();
        const onlyLiteralsBetweenBrackets = this.checkOnlyLiteralsBetweenBrackets();
    ...
    checkParenthesesCountMatches(parentheses) {
    ...
    checkParenthesesOrdering(parentheses, violations =[]) {
    ...
    checkBracketCountMatches(brackets) {
    ...
    checkCurlyBraceCountMatches(curlyBraces) {
    ...
    checkCurlyBraceOrdering(curlyBraces) {
        let curlyBraceStack = [];
    ...
    checkConditionalCountMatches(tokens) {
    ...
    checkOnlyLiteralsBetweenCurlyBraces() {
        let stack = [];
        for (let i = 0; i < this.tokens.length; i++) {
            ...
            if (token.type === 'openCurlyBrace') {
                ...
                logError(`...${this.tokens.slice(i - 2, i + 3).map((t) => t.value).join(' ')}...`);
                return false;
            }
        }
    ...
    checkOnlyLiteralsBetweenBrackets() {
        let stack = [];
        for (let i = 0; i < this.tokens.length; i++) {
            const token = this.tokens[i];
            if (token.type === 'openBracket') {
